...
static int panel_jdi_soft_reset(struct panel_jdi *jdi)
{
	int ret;

	ret = mipi_dsi_dcs_write(jdi->dsi,
			MIPI_DCS_SOFT_RESET, NULL, 0);
	if (ret < 1) {
		DRM_ERROR("failed to soft-reset the display: %d\n", ret);
		return ret;
	}
...
static int panel_jdi_write_display_brightness(struct panel_jdi *jdi)
{
	int ret;
	u8 data;
	data = RSP_WRITE_DISPLAY_BRIGHTNESS(0x55);
	ret = mipi_dsi_dcs_write(jdi->dsi,
...
static int panel_jdi_prepare(struct drm_panel *panel)
{
	...
	gpio_set_value(jdi->enable_gpio, (jdi->enable_gpio_flags & GPIO_ACTIVE_LOW) ? 1 : 0);
	/* T2 = 10ms */
	usleep_range(10000, 15000);

	gpio_set_value(jdi->enable_gpio, (jdi->enable_gpio_flags & GPIO_ACTIVE_LOW) ? 0 : 1);
	/* T2 = 10ms */
	usleep_range(10000, 15000);

  // XXX Try to force a display reset...
	gpio_set_value(jdi->reset_gpio, (jdi->reset_gpio_flags & GPIO_ACTIVE_LOW) ? 0 : 1);
  msleep(1000);


	gpio_set_value(jdi->reset_gpio, (jdi->reset_gpio_flags & GPIO_ACTIVE_LOW) ? 1 : 0);

  panel_jdi_soft_reset(jdi);
  msleep(1000);
...
static int panel_jdi_get_modes(struct drm_panel *panel)
{
  ...
  panel->connector->display_info.tearing_effect = 0;
	return 1;
}
